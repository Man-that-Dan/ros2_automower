...
BladeController::BladeController()
{
  ...
  cfg_.enc_counts_per_rev = this->get_parameter("enc_counts_per_rev").get_value<int>();

  blade_status_publisher =
    this->create_publisher<ros2_automower_control::msg::BladeStatus>("blade_status", 1);
  ...
  publish_timer = this->create_wall_timer(
    std::chrono::seconds(0.2), std::bind(&BladeController::publishBladeStatus, this));
  command_timer = this->create_wall_timer(
    std::chrono::seconds(0.2), std::bind(&BladeController::sendBladeCommand, this));
}
